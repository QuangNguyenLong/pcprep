                ...
                vec3uc_t bg_col)
{
    ...
    cv->height = height;
    cv->bg_col = bg_col;
    cv->pixels = (unsigned char *)malloc
        (sizeof(unsigned char) *
         cv->width *
         cv->height *
         3);
    cv->min_z_value = (float **)malloc
        (sizeof(float *) *
         cv->height);
    for(int i = 0; i < cv->height; i++)
    {
        cv->min_z_value[i] = (float *)malloc
            (sizeof(float) *
             cv->width);
    }
#ifdef HAVE_GPU
                            ...
                            size_t count)
{
    ...
    for (int i = 0; i < count; i++)
    {
            ...
            ndc.z >= 0  && ndc.z <= 1)
        {
            ...
            if (screen_w >= cv->width) screen_w = cv->width - 1;
            if (screen_h >= cv->height) screen_h = cv->height - 1;
...
void canvas_clear(canvas_t *cv)
{
    ...
    for(int i = 0; i < cv->height; i++)
    {
    memset(cv->min_z_value[i], 
           0, 
           sizeof(float) * cv->width);
    }
}
